...
# from actions.chat_append import ChatHistory_Response
# from Action.webbot_chathistory import *
import asyncio
...
class SocketIOInput(InputChannel, Action):
    ...
    #     # x=SocketIOOutput()
    #     # sio=x.sio
    #     # bot_message_evt=x.bot_message_evt
    #     await self.sio.emit(self.bot_message_evt, response, room=socket_id)
    ...
    def blueprint(
        ...
        async def disconnect(sid: Text) -> None:
            ...
            # save_chat_Responce =ChatHistory_Response.chat_log()
            initial_chat={
                            "company":"",
                            "id":"",
                            "chatDate":"",
            ...
            # x=SocketIOOutput()
            # sid=x.sid
            # message={"message": "/bot_closed"}
            # await self.emit(sid,message )
            # if data is None:
            #     data = {}
            # if "session_id" not in data or data["session_id"] is None:
            #     data["session_id"] = uuid.uuid4().hex
            # data["message"]="/bot_closed"
        ...
        async def handle_message(sid: Text, data: Dict) -> Any:
            ...
            ui_message = data
            logger.info(ui_message)
            json_object = json.dumps(data, indent=4)
            # Writing to sample.json
            with open("ui_message.json", "w") as outfile:
                outfile.write(json_object)
            # print(ui_message,"ui message2222222222222222222")
            # print(data,"data from ui 111111111111111111111111111111")
            # if data["message"] =='/greet_user':
                # chat_history(data["message"],sender_id)
            # else:
                # chat_history2(data["message"],sender_id)
